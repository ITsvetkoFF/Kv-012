...
var mockData = (function() {
    ...
    function getFilterDirectiveTmpl(){
        return '<smart-filter type="runs">'
                    +'<div class="input-group" uib-dropdown is-open="status.isopen">'
                        +'<buttton class="input-group-addon" uib-dropdown-toggle>'
                            +'Filters'
                            +'<span class="caret"></span>'
                        +'</buttton>'
                        +'<ul uib-dropdown-menu role="menu" aria-labelledby="single-button">'
                            +'<li role="menuitem" ng-click="$parent.filterQuery=\'status=failed\'"><a href="#">status=failed</a></li>'
                            +'<li role="menuitem" ng-click="$parent.filterQuery=\'status=passed\'"><a href="#">status=passed</a></li>'
                            +'<li role="menuitem" ng-click="$parent.filterQuery=\'date> month ago\'"><a href="#">date> month ago</a></li>'
                            +'<li role="menuitem" ng-click="$parent.filterQuery=\'date> year ago\'"><a href="#">date> year ago</a></li>'
                        +'</ul>'
                        +'<input type="text" class="form-control" placeholder="Filter" ng-model="$parent.filterQuery" typeahead-wait-ms="150"'
                            +'uib-typeahead="key for key in hintKeys" typeahead-input-formatter="inputFormatter($model)">'
                    +'</div>'
                +'</smart-filter>';
    }
